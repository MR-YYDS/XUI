...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/MR-YYDS/XUI/refs/heads/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://raw.githubusercontent.com/MR-YYDS/XUI/refs/tags/YYDS/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
